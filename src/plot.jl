...
function plot_co_occurrence_matrix(markov_chains::Array{Int64}; plot_size=600)
    ...
    plot(subplots..., layout = (num_chains, 1), legend=false, size = (plot_size, plot_size * num_chains*(3/4)))
end
...
function plot_num_clusters(markov_chain::Matrix{Int64}) 
...
function plot_ari_posterior_distribution(true_clustering::Vector{Int64}, assignment_posterior::Array{Int64})
    num_chains = size(assignment_posterior)[3]
    subplots = []
    for chain = 1:num_chains
        ari_posterior = ari_over_markov_chain(true_clustering, assignment_posterior[:, :, chain])
        mean_ari = round(mean(ari_posterior), digits=3)
